...
module.exports = async function lint (args = {}, api) {
  ...
  const { log, done, exit, chalk, loadModule } = require('jslib-util')
  ...
  const fixErrorsPredicate = (lintResult) => lintResult.severity === 2
  config.fix = config.fix && (argsConfig.fixErrors ? fixErrorsPredicate : true)
