  ...
  new Generator('/', { plugins: [
    {
      ...
      apply: api => {
        ...
        expect(api.hasPlugin('jslib-plugin-baz')).toBe(true)
      }
    },
    {
      id: 'jslib-plugin-bar',
...
test('api: addConfigTransform transform jslib warn', async () => {
  const configs = {
    jslib: {
      lintOnSave: true
    }
  ...
  expect(fs.readFileSync('/jslib.config.js', 'utf-8')).toMatch('module.exports = {\n  lintOnSave: true\n}')
  expect(logs.warn.some(([msg]) => {
    return msg.match(/Reserved config transform 'jslib'/)
  })).toBe(true)
})
...
test('extract config files', async () => {
  ...
  const js = v => `module.exports = ${stringifyJS(v, null, 2)}`
  expect(fs.readFileSync('/jslib.config.js', 'utf-8')).toMatch(js(configs.jslib))
