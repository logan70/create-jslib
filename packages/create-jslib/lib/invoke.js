...
function getPkg (context) {
  ...
  if (pkg.jslibPlugins && pkg.jslibPlugins.resolveFrom) {
    return getPkg(path.resolve(context, pkg.jslibPlugins.resolveFrom))
  }
  return pkg
}
...
async function invoke (pluginName, options = {}, context = process.cwd()) {
  ...
  const findPlugin = deps => {
    ...
    let name
    // official
    if (deps[(name = `jslib-plugin-${pluginName}`)]) {
      return name
    }
...
async function runGenerator (context, plugin, pkg = getPkg(context)) {
  ...
  if (!process.env.JSLIB_TEST && hasProjectGit(context)) {
