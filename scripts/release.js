...
process.env.JSLIB_RELEASE = true
...
const release = async () => {
  ...
  const versionMarkerPath = path.resolve(__dirname, '../packages/jslib-version-marker/package.json')
  ...
  const tagName = `jslib-version-marker@${versionMarkerPkg.version}`
  await execa('git', ['add', '-A'], { stdio: 'inherit' })
