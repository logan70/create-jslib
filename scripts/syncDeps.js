...
const externalJSLibScopedPackages = {
  'jslib-test-utils': true
}
const localPackageRE = /'([\w-.]*jslib[\w-]+)': '\^([\w-.]+)'/g
  ...
  if (!local) {
    ...
    const resolvedPackages = (await Promise.all(packages.filter(filePath => {
      ...
      for (const dep in deps) {
        if (dep.match(/jslib/) && !externalJSLibScopedPackages[dep]) {
          continue
        }
        let local = deps[dep]
        if (local.charAt(0) !== '^') {
    ...
    for (const { pkg, filePath, resolvedDeps } of resolvedPackages) {
      debug(`jslib:syncDeps-filePath`)(filePath)
      debug(`jslib:syncDeps-resolvedDeps`)(resolvedDeps)
      let isUpdated = false
  ...
  paths
    ...
    .forEach(filePath => {
      ...
      const updated = fs.readFileSync(filePath, 'utf-8')
        // update jslib packages in this repo
        .replace(localPackageRE, localReplacer)
        // also update remote packages
